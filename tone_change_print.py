...
f = open('text.txt')
fo = open('M1變調流程.txt', 'w')
for line in f.readlines():
    # print('變調前→' + line.strip())
    fo.write('變調前→' + line.strip() + '\n' + '~~~~~~~~~~~~~~~~~~~' + '\n')
    taibun = '###' + line
    ...
    # print('句物件→' + str(句物件))
    fo.write('句物件→' + str(句物件) + '\n' + '~~~~~~~~~~~~~~~~~~~' + '\n')
    結果句物件 = 句物件.轉音(臺灣閩南語羅馬字拼音, 函式='音值')
    # print('結果句物件→' + str(結果句物件))
    fo.write('結果句物件→' + str(結果句物件) + '\n' + '~~~~~~~~~~~~~~~~~~~' + '\n')
    判斷陣列 = 變調判斷.判斷(結果句物件)
    # print('判斷陣列→' + str(判斷陣列))
    fo.write('判斷陣列→' + str(判斷陣列) + '\n' + '~~~~~~~~~~~~~~~~~~~' + '\n')
    i = 0
    for 詞物件, 原底詞 in zip(結果句物件.網出詞物件(), 句物件.網出詞物件()):
        ...
        for 字物件, 原底字 in zip(詞物件.內底字, 原底詞.內底字):
            變調方式 = 判斷陣列[i]
            # print('變調方式→' + str(變調方式))
            fo.write('變調方式→' + str(變調方式) + '\n' + '~~~~~~~~~~~~~~~~~~~' + '\n')
            if 變調方式 == 變調判斷.愛提掉的:
                pass
            else:
                if 字物件.音 == (None,):
                    新陣列.append(原底字.khóopih字())
                    # print('原底字.khóopih字()→' + str(原底字.khóopih字()))
                    fo.write('原底字.khóopih字()→' + str(原底字.khóopih字()) + '\n' + '~~~~~~~~~~~~~~~~~~~' + '\n')
                else:
